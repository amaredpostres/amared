// =================== CONFIG ===================
const API_URL = window.AMARED_CONFIG.apiUrl;
...
async function api(body, retries = 2) {
  ...
  const res = await window.amaredRequest(API_URL, {
    method: "POST",
...
function renderOrdersList(container, orders, { mode }) {
  if (!container) return;
  if(mode === "PENDIENTES" && container.id === "list") {
    const query=String(document.getElementById("orderSearch")?.value || "").trim().toLocaleLowerCase("es");
    if(query) orders=(orders || []).filter(order=>[order.customer_name,order.phone,order.order_id].some(value=>String(value || "").toLocaleLowerCase("es").includes(query)));
  }
  container.innerHTML = "";

  if (!orders || orders.length === 0) {
    container.innerHTML = `<div class="orderEmpty"><strong>No hay pedidos en esta vista</strong>Prueba otra búsqueda o actualiza para consultar los últimos cambios.</div>`;
    return;
  }
